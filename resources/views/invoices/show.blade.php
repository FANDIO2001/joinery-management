            ...
            <div style="display:grid; grid-template-columns:1fr 380px; gap:32px;">
                ...
                <div>
                        ...
                        <div style="display:flex; gap:24px; align-items:flex-start;">
                            ...
                            <div style="flex:1;">
                                ...
                                <p style="color:#6b7280; margin:0 0 16px; font-size:16px;">
                                    Facture pour {{ $invoice->order->client->name ?? 'Client inconnu' }}
                                </p>
                                <div style="display:flex; gap:20px; flex-wrap:wrap;">
                                    ...
                                    <div style="display:flex; align-items:center; gap:8px;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-size:14px; font-weight:600; color:#1f2937;">
                                                {{ $invoice->due_date->format('d/m/Y') }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div style="padding:0 24px 24px;">
                            <div style="display:flex; align-items:center; gap:16px;">
                                <div
                                    ...
                                    {{ strtoupper(substr($invoice->order->client->name ?? 'C', 0, 2)) }}
                                </div>
                                <div>
                                    <div style="font-size:15px; font-weight:600; color:#1f2937;">
                                        {{ $invoice->order->client->name ?? 'Non renseigné' }}</div>
                                    <div style="font-size:13px; color:#6b7280;">
                                        {{ $invoice->order->client->email ?? 'Non renseigné' }}</div>
                                    <div style="font-size:13px; color:#6b7280;">
                                        {{ $invoice->order->client->phone ?? 'Non renseigné' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div style="padding:0 24px 24px;">
                            <div style="font-size:15px; font-weight:600; color:#1f2937;">
                                {{ $invoice->order->reference ?? 'Aucune commande' }}</div>
                            <div style="font-size:13px; color:#6b7280;">
                                {{ optional($invoice->order->items)->count() ?? 0 }} article(s)</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div
                        ...
                        <div style="padding:0 24px 24px;">
                            @php
                                ...
                                $status = $statusColors[$invoice->status] ?? $statusColors['draft'];
                            @endphp
                            <span
                                style="display:inline-flex; align-items:center; gap:6px; padding:6px 16px; background:{{ $status['bg'] }}; color:{{ $status['text'] }}; font-size:14px; font-weight:600; border-radius:20px;">
                                <div style="width:8px; height:8px; background:{{ $status['dot'] }}; border-radius:50%;">
                                </div>
                                {{ $status['label'] }}
                            </span>
                        </div>
                    ...
                    <div
                        ...
                        <div style="padding:0 24px 24px; text-align:center;">
                            <div style="font-size:32px; font-weight:700; color:#1f2937; margin-bottom:4px;">
                                {{ number_format($invoice->total_amount, 0, ',', ' ') }} FCFA</div>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div style="padding:0 24px 24px; display:flex; flex-direction:column; gap:12px;">
                            @if (auth()->user()->user_type === 'admin')
                                ...
                                <button
                                    style="padding:12px 16px; background:linear-gradient(135deg, #3b82f6, #2563eb); color:white; border:none; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; transition:all 0.2s; box-shadow:0 4px 12px rgba(59,130,246,0.3);"
                                ...
                                <button
                                    style="padding:12px 16px; background:linear-gradient(135deg, #ef4444, #dc2626); color:white; border:none; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; transition:all 0.2s; box-shadow:0 4px 12px rgba(239,68,68,0.3);"
                                ...
                                <div style="position:relative; margin-bottom:16px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-size:12px; color:#6b7280;">
                                            {{ $invoice->created_at->format('d/m/Y à H:i') }}</div>
                                    </div>
                                </div>
                                <div style="position:relative;">
                                    ...
                                    <div>
                                        <div style="font-size:13px; font-weight:600; color:#1f2937; margin-bottom:2px;">
                                            Paiement reçu</div>
                                        <div style="font-size:12px; color:#6b7280;">
                                            {{ $invoice->paid_at ? $invoice->paid_at->format('d/m/Y à H:i') : 'En attente' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
